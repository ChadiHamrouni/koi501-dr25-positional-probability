...
FRAMES = config.CONTRAST_FRAMES
SIGMA = config.CONTRAST_SIGMA
...
def radial_fwhm(img, yc, xc, scale):
    ...
    background = np.median(img[r > config.CONTRAST_BACKGROUND_FRACTION * min(img.shape)])
    profile = img - background
    peak = profile[int(round(yc)), int(round(xc))]
    r_max, n_radii, width = config.CONTRAST_PROFILE
    radii = np.linspace(0, r_max, n_radii)
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", RuntimeWarning)   # rings finer than a pixel can be empty
        values = np.array([np.mean(profile[(r >= a) & (r < a + width)]) for a in radii])
...
def contrast_curve(img, header: FitsImageHeader, seeing_as: float) -> dict:
    ...
    ny, nx = img.shape
    yc, xc = centroid(img, (ny / 2, nx / 2), box=max(config.CONTRAST_CENTROID_BOX[0], int(config.CONTRAST_CENTROID_BOX[1] / scale)))
    ...
    ring_median = np.array([np.median(profile[ring == i]) if (ring == i).sum() > config.CONTRAST_MIN_RING_PIXELS else 0.0
    ...
    separations, limits = [], []
    for sep in np.arange(max(fwhm_as, 2 * aperture_px * scale), config.CONTRAST_OUTER_FRACTION * nx * scale,
                         max(0.5 * fwhm_as, scale)):
        rr = sep / scale
        n_apertures = max(config.CONTRAST_MIN_APERTURES, int(2 * np.pi * rr / (2 * aperture_px)))
        fluxes = []
        ...
        if len(fluxes) < config.CONTRAST_MIN_FLUXES:
            continue
        fluxes = np.array(fluxes)
        scatter = config.MAD_TO_SIGMA * np.median(np.abs(fluxes - np.median(fluxes)))
...
def main() -> bool:
    ...
    near = int(np.argmin(np.abs(sep - config.CONTRAST_QUOTE_SEP_AS)))
