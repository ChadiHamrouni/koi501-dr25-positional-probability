...
over the 29.4-minute exposure is fitted to the flattened transits, then refitted
...
PERIOD = config.SHAPE_PERIOD_D
EPOCH = config.EPOCH_BKJD
DURATION = config.SHAPE_DURATION_H / 24.0
DEPTH = config.SHAPE_DEPTH_PPM * 1e-6
CADENCE = config.LONG_CADENCE_MIN / 60.0 / 24.0
BOUNDS = ([-0.05, 1e-5, 0.1, 1e-4], [0.05, 0.05, 0.6, 0.25])
...
def flattened_transits() -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    near = np.abs(phase) < 0.06
    xs, ys, es = [], [], []
    for k in np.unique(epoch[near]):
        window = near & (epoch == k)
        if window.sum() < 20:
            continue
        out = window & (np.abs(phase) > 0.75 * DURATION / PERIOD)
        if out.sum() < 10:
            continue
...
def trapezoid(x, t0, depth, T, tau):
    ...
    offsets = np.linspace(-0.5, 0.5, 9) * CADENCE
    y = np.zeros((len(offsets), len(x)))
    for i, o in enumerate(offsets):
        u = np.abs(x - t0 + o)
        flat = 0.5 * max(T - 2 * tau, 1e-6)
        outer = flat + tau
        v = np.ones_like(u)
        v[u <= flat] = 1 - depth
        ramp = (u > flat) & (u < outer)
        v[ramp] = 1 - depth * (outer - u[ramp]) / max(tau, 1e-6)
        y[i] = v
...
def percentiles(values, scale=1.0) -> list[float]:
    return [float(np.percentile(values, p) * scale) for p in (16, 50, 84)]
...
def main() -> bool:
    ...
    best, _ = curve_fit(trapezoid, x, y, p0=[0.0, 575e-6, DURATION, 0.05], sigma=e,
    ...
    shape_bound = float(np.percentile(dmag, 95))
    ...
    payload = {
        "light_curve": {"file": LIGHT_CURVE.relative_to(config.ROOT).as_posix(),
                        ...
                        "exposure_min": config.LONG_CADENCE_MIN},
        "best_fit": {"epoch_offset_min": float(best[0] * 1440), "depth_ppm": float(best[1] * 1e6),
        ...
        "bootstrap": {"requested": config.SHAPE_BOOTSTRAPS, "converged": int(len(fits)),
                      ...
                      "depth_ppm_p16_p50_p84": percentiles(depths, 1e6),
        ...
        "max_true_depth_p95_ppm": float(np.percentile(dmax, 95) * 1e6),
    ...
    table("Gaia DR3 sources within 25 arcsec besides the target", len(neighbours))
    table("  brighter than the shape bound", sum(r["shape_allows"] for r in neighbours))
    table("  within 3 standard errors of the centroid",
