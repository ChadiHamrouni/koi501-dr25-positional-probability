...
Inputs
  ...
  star         radius, mass and Teff of step 07 (results/07_host_star.json)
               and the Gaia DR3 parallax of step 01
  transit      depth, period and epoch from the DR25 detection table (step 00);
               long-cadence exposure from koi501/config.py
...
KEPID = config.KEPID
USED = ["transit.period_tce_d", "transit.epoch_bkjd", "transit.duration_h", "transit.depth_ppm",
        "star.radius_rsun", "star.mass_msun", "star.teff_k", "star.logg", "star.parallax_mas"]
PERIOD_D, EPOCH_BKJD = inputs.get("transit.period_tce_d"), inputs.get("transit.epoch_bkjd")
DURATION_H, DEPTH_PPM = inputs.get("transit.duration_h"), inputs.get("transit.depth_ppm")
EXPTIME_D = config.LONG_CADENCE_D
R_STAR, M_STAR = inputs.get("star.radius_rsun"), inputs.get("star.mass_msun")
TEFF, LOGG = inputs.get("star.teff_k"), inputs.get("star.logg")
SEARCH_RADIUS_PX = config.TRICERATOPS_SEARCH_RADIUS_PX
N_RUNS, N_DRAWS, SEED = config.TRICERATOPS_RUNS, config.TRICERATOPS_DRAWS, config.TRICERATOPS_SEED
FPP_BAR, NFPP_BAR = config.TRICERATOPS_FPP_BAR, config.TRICERATOPS_NFPP_BAR
FOLD = config.TRICERATOPS_FOLD
...
def folded_light_curve(window: float = FOLD["window"], nbin: int = FOLD["bins"]):
    ...
    lc = lc.flatten(window_length=FOLD["flatten"], mask=np.abs(phase) < dur).remove_outliers(
        sigma_upper=FOLD["clip"][0], sigma_lower=FOLD["clip"][1])
    phase = (lc.time.value - EPOCH_BKJD + 0.5 * PERIOD_D) % PERIOD_D - 0.5 * PERIOD_D
...
def one_run(k: int, lc, prepared, cc_file: Path) -> dict:
    ...
    target.calc_depths(tdepth=DEPTH_PPM / config.PPM, all_ap_pixels=aperture)
...
def main() -> int:
    ...
    parallax = inputs.get("star.parallax_mas")
    cc_file = contrast_curve(out)
    lc = folded_light_curve()
    # the same folded curve is vespa's photometry (fpp/vespa/make_inputs.py)
    np.savetxt(out / "fpp_triceratops_lc.csv",
               np.column_stack([lc[0], lc[1], np.full(len(lc[0]), lc[2])]), fmt="%.8f", delimiter=",")
    quarter = first_quarter()
    print(f"folded light curve: {len(lc[0])} bins, out-of-transit scatter "
          f"{lc[2] * config.PPM:.0f} ppm; target pixel file from quarter {quarter}")
